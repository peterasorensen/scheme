...
def do_and_form(expressions, env):
    ...
    while(expressions):
        if not expressions.second:
            current_expr = scheme_eval(expressions.first, env, True)
        else:
            current_expr = scheme_eval(expressions.first, env)
        if scheme_false(current_expr):
            return False
...
def check_formals(formals):
    ...
    symbols = []
    while formals != nil:
        if formals.first in symbols or not scheme_symbolp(formals.first):
            raise SchemeError("List of formal parameters passed is invalid")
